<html lang="en">
 ...
 <body class="bg-gray-100 min-h-screen flex">
     ...
     <aside id="sidebar" class="bg-teal-800 w-64 transition-all duration-300 ease-in-out min-h-screen flex flex-col">
      ...
      <nav id="sidebar" class="flex flex-col mt-6 text-white/70 text-sm space-y-1 group">
        <a class="flex hover:bg-teal-700 items-center gap-3 px-7 py-6 hover:text-white transition-colors
         {{ request()->routeIs('supkorla.dashboard-page') ? 'bg-teal-700 text-white' : '' }}"
      ...
      <a class="flex hover:bg-teal-700 items-center gap-5 px-7 py-6 hover:text-white transition-colors
         {{ request()->routeIs('') ||
           request()->routeIs('supkorla.daftar-pengajuan-surat-page') ? 'bg-teal-700 text-white' : '' }}"
       ...
       <a class="flex hover:bg-teal-700 items-center gap-5 px-7 py-6 hover:text-white transition-colors
         {{ request()->routeIs('') ||
           request()->routeIs('supkorla.daftar-ruangan-page') ? 'bg-teal-700 text-white' : '' }}"
  ...
  <img alt="Profile picture" class="w-10 h-10 rounded-full object-cover" src="{{ $profile && $profile->image ? asset('storage/' . $profile->image) : asset('assets/images/icon/none-profile-icon.svg') }}" />
  <div class="text-right">
    <p class="text-gray-900 font-semibold text-sm leading-tight">{{ $profile ? $profile->name : '' }}</p>
    <p class="text-gray-400 text-xs leading-tight">{{ $profile ? Str::upper($profile->role) : '' }} PUSTIKOM</p>
  </div>
</button>

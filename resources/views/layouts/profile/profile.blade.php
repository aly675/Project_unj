@php
    $layout = 'layouts.layout'; // Default layout
    if ($profile) {
        switch ($profile->role) {
            case 'superadmin':
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex flex-col lg:flex-row gap-4">
        ...
        <div class="bg-gray-200 rounded-lg p-6 flex flex-col items-center justify-center w-full lg:w-1/3">
          <div class="relative">
            <img src="{{ $profile && $profile->image ? asset('storage/' . $profile->image) : asset('assets/images/icon/none-profile-icon.svg') }}" class="w-40 h-40 rounded-full border-4 border-white shadow object-cover" alt="Foto Profil">
            <span class="absolute -top-2 -right-2 bg-green-200 text-green-700 text-xs px-2 py-1 rounded-full">{{ $profile ? $profile->status : '' }}</span>
          </div>
        </div>
        ...
        <div class="bg-gray-200 rounded-lg p-6 w-full lg:w-2/3">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" class="w-full p-2 rounded border border-gray-300 text-sm" value="{{ $profile ? $profile->name : '' }}" placeholder="Nama Lengkap">
            </div>
            <div>
              <label class="text-xs text-gray-600 block mb-1">Role:</label>
              <input type="text" class="w-full p-2 rounded border border-gray-300 text-sm" disabled value="{{ $profile ? $profile->role : '' }}" placeholder="Username">
            </div>
            <div>
              <label class="text-xs text-gray-600 block mb-1">Email:</label>
              <input type="email" class="w-full p-2 rounded border border-gray-300 text-sm" value="{{ $profile ? $profile->email : '' }}" placeholder="Email">
            </div>
            <div>
